...
//                }
    ...
    public String stringMyVendingMachines() {
        String str = "\nMy vending Machines";

        for (Map.Entry<String, ArrayList<Machine>> map : myVMachinesMap.entrySet()) {
            str += "\n"+map.getKey();
            for (Machine machine : map.getValue()) {
                str += "\tVending Machine: [" + machine.getMachine_id() + "] " + machine.getCoin().getTotalAmountMoney();
            }
        }
        return str;
    }
    ...
    public Map<String, ArrayList<Machine>> getMyVMachinesMap() {
        return myVMachinesMap;
    }
}
